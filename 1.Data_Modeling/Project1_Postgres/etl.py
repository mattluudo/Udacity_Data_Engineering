...
def process_song_file(cur, filepath):
    """Read JSON song data and inserts it into artists and songs Sparkify Postgres tables.

    Keyword arguments:
    cur -- Cursor to Sparkify database
    filepath -- Root directory of song files
    """
    ...
    for x in range(len(df)):
        # insert song record
        song_data = df[["song_id", "title", "artist_id", "year", "duration"]].values[x].tolist()
        cur.execute(song_table_insert, song_data)
 
        # insert artist record
        artist_data = df[["artist_id", "artist_name", "artist_location", "artist_latitude",
                          "artist_longitude"]].values[x].tolist()
        cur.execute(artist_table_insert, artist_data)


def process_log_file(cur, filepath):
    """Read JSON log data and inserts it into time, users and songplays Sparkify Postgres tables.
...
def process_data(cur, conn, filepath, func):
    """Performs either process_log_file or process_song_file function

    Keyword arguments:
    cur -- Cursor to Sparkify database
    conn -- Connection to Sparkify database
    filepath -- Root directory of data
